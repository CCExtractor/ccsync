...
func main() {
	...
	authMiddleware := middleware.RequireAuth(store)
	...
	mux.Handle("/tasks", rateLimitedHandler(authMiddleware(http.HandlerFunc(controllers.TasksHandler))))
	mux.Handle("/add-task", rateLimitedHandler(authMiddleware(http.HandlerFunc(controllers.AddTaskHandler))))
	mux.Handle("/edit-task", rateLimitedHandler(authMiddleware(http.HandlerFunc(controllers.EditTaskHandler))))
	mux.Handle("/modify-task", rateLimitedHandler(authMiddleware(http.HandlerFunc(controllers.ModifyTaskHandler))))
	mux.Handle("/complete-task", rateLimitedHandler(authMiddleware(http.HandlerFunc(controllers.CompleteTaskHandler))))
	mux.Handle("/delete-task", rateLimitedHandler(authMiddleware(http.HandlerFunc(controllers.DeleteTaskHandler))))
	mux.Handle("/sync/logs", rateLimitedHandler(http.HandlerFunc(controllers.SyncLogsHandler)))
	mux.Handle("/complete-tasks", rateLimitedHandler(authMiddleware(http.HandlerFunc(controllers.BulkCompleteTaskHandler))))
	mux.Handle("/delete-tasks", rateLimitedHandler(authMiddleware(http.HandlerFunc(controllers.BulkDeleteTaskHandler))))
