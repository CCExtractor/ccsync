...
export const DevLogs: React.FC<DevLogsProps> = ({ isOpen, onOpenChange }) => {
    ...
    const copyLog = (log: LogEntry, index: number) => {
        const logText = `[${log.timestamp}] [${log.level}] ${log.message}${log.operation ? ` | Operation: ${log.operation}` : ''
    ...
    const copyAllLogs = () => {
        const allLogsText = filteredLogs
            .map(
                (log) =>
                    `[${log.timestamp}] [${log.level}] ${log.message}${log.operation ? ` | Operation: ${log.operation}` : ''
                    }${log.syncId ? ` | Sync ID: ${log.syncId}` : ''}`
            )
            .join('\n');
